			...
			<div class="bm-single-computer-container">
				...
				<div class="bm-single-computer-container-info">
					...
					<div class="bm-single-product-price-buttons">
						...
						<div class="bm-single-product-price-buttons__addCart">
							<?php // do_action( 'woocommerce_' . $product->get_type() . '_add_to_cart' );?>
							<?php if ( $product->get_stock_status() == 'onbackorder'){ ?>
								<a href="<?php echo $product->add_to_cart_url(); ?>" data-product_id="<?php echo $product->get_id(); ?>"  rel="nofollow" class="single_add_to_cart_button button product_type_simple add_to_cart_button ajax_add_to_cart">Предзаказ</a>	
									
							<?php }else{ ?>
								<a href="<?php echo $product->add_to_cart_url(); ?>" data-product_id="<?php echo $product->get_id(); ?>"  rel="nofollow" class="single_add_to_cart_button button product_type_simple add_to_cart_button ajax_add_to_cart">В корзину</a>	
							<?php } ?>
							
							
						</div>
						<div class="bm-single-product-price-buttons_oneClick">
							<?php 
								if (get_field("podarok")){ ?>
									<p class="bm-single-product-price-buttons_oneClick-gift">
										Мышка в подарок
									</p>
									<style>
										.bm-single-product-price-buttons_oneClick{margin-left: 20px;} .bm-single-product-price-buttons_oneClick-gift{margin: 0;}
									</style>
								<?php }
							?>
					...
					<?php if(get_field("vybor_ssd_m2")){ ?>
						<?php 
							switch ( get_field('vybor_ssd_m2') ){
								...
								<?php }		
								case "tb2":
									if ( get_field('ssd_m2_2tb') ){ ?>
										<div class="bm-single-product-setup-container-set-block">
											<p class="bm-single-product-setup-container-set-block__title">
												SSD M2
											</p>
											<p class="bm-single-product-setup-container-set-block__text ssd_nakopitel-attr">
												<?php the_field('ssd_m2_2tb') ?>
											</p>
										</div>											
								<?php }										
							} ?>					
...
<?php 
	...
	$user_female = $current_user->user_lastname;
?>
...
<script>
	...
	$(function() {
		...
		$(".pdf").click(function(){
			...
			var ManName = '<?php echo $user_name; ?>';
			var ManFem = '<?php echo $user_female; ?>';
			...
			var productPrice = $(".bm-single-computer-container-info bdi").text();
			...
			arrayConfig.push([ { text: 'Менеджер: \n' + ManName + ' ' + ManFem, bold: true , color: "#fff", fontSize: 12, alignment: "left", lineHeight: 1.3, margin: [ 0, 10, 0, 10 ]}, { text: '8 (800) 350-83-39', color: "#fff", fontSize: 12, lineHeight: 1, margin: [ 0, 10, 0, 10 ]} ]);
