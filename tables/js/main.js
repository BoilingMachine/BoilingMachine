jQuery(document).ready(function($){
		...
		this.steps.each(function(){
			...
			if( $(this).data('selection') == 'colors' ) {
				...
				if( selectedOptions.length == 0 ) {
					optionsContent = '<li><p>Комплектубщие не выбраны</p></li>';
				} else {
					selectedOptions.each(function(){
						optionsContent +='<li class="summary-spisok-charac"><span class="name">'+$(this).find('p').attr('data-product-name')+'</span><span class="value">'+$(this).find('p').attr('data-product-value')+'</span></li>';
					});
				}
	...
	/*
	$('.options-container-body .js-option').click(function(){
		console.log('clickerrr');
		var options = $(this).parent().find('.js-option');
